  ...
  useGSAP(
    () => {
      ...
      document.fonts.ready.then(() => {
        ...
        if (microItems.length > 0 && microDetails && microDetails.length > 0) {
    ...
    <section ref={sectionRef} className="relative h-screen w-screen overflow-hidden">
      ...
      <nav className="relative z-10 flex items-center justify-between px-8 py-8 md:px-12 lg:px-20">
        ...
        <div className="hidden md:flex items-center gap-12">
          ...
          <a href="#contact" className="text-base font-light tracking-tight text-white/80 transition-colors hover:text-white">
            Contact Us
          </a>
          <LoginDropdown />
        </div>
      ...
      <div className="relative mx-auto flex h-full max-w-6xl flex-col items-center justify-center gap-8 px-6 text-center md:px-10 lg:px-16 -mt-20">
        {badgeText && (
          <div ref={badgeRef} className="inline-flex items-center gap-3 rounded-full border border-white/10 bg-white/5 px-4 py-2 backdrop-blur-sm">
            <span className="text-xs font-light uppercase tracking-[0.08em] text-white/70">{badgeLabel}</span>
            <span className="h-1.5 w-1.5 rounded-full bg-white/40" />
            <span className="text-sm font-light tracking-tight text-white/80">{badgeText}</span>
          </div>
        )}

        <h1 ref={headerRef} className="max-w-5xl text-center text-6xl font-extralight leading-[1.05] tracking-tight text-white sm:text-7xl md:text-8xl lg:text-9xl">
          {title}
        </h1>

        <p ref={paraRef} className="max-w-3xl text-center text-lg font-light leading-relaxed tracking-tight text-white/75 sm:text-xl md:text-2xl">
          {description}
        </p>
        ...
        <div ref={ctaRef} className="flex flex-wrap items-center justify-center gap-4 pt-4">
          {ctaButtons.map((button, index) => (
            <a
              key={index}
              href={button.href}
              className="rounded-2xl border border-white/20 bg-white/10 px-8 py-4 text-base font-light tracking-tight text-white backdrop-blur-sm transition-colors hover:bg-white/20 focus:outline-none focus:ring-2 focus:ring-white/30 duration-300"
            >
              {button.text}
            </a>
          ))}
        ...
        {microDetails && microDetails.length > 0 && (
          <ul ref={microRef} className="mt-12 flex flex-wrap justify-center gap-8 text-sm font-extralight tracking-tight text-white/60">
            {microDetails.map((detail, index) => {
              const refMap = [microItem1Ref, microItem2Ref, microItem3Ref];
              return (
                <li key={index} ref={refMap[index]} className="flex items-center gap-3">
                  <span className="h-1.5 w-1.5 rounded-full bg-white/40" /> {detail}
                </li>
              );
            })}
          </ul>
